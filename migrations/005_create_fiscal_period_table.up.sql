-- Migration: Create fiscal_periods table
-- Generated by backbone-schema

CREATE TABLE IF NOT EXISTS fiscal_periods (
    id UUID NOT NULL DEFAULT gen_random_uuid(),
    provider_id UUID,
    period_code TEXT NOT NULL,
    name TEXT NOT NULL,
    period_type period_type NOT NULL DEFAULT 'monthly',
    start_date DATE NOT NULL,
    end_date DATE NOT NULL,
    fiscal_year INTEGER NOT NULL,
    fiscal_quarter INTEGER,
    fiscal_month INTEGER,
    parent_id UUID,
    level INTEGER NOT NULL DEFAULT 0,
    status period_status NOT NULL DEFAULT 'open',
    is_current BOOLEAN NOT NULL DEFAULT FALSE,
    opening_balance_set BOOLEAN NOT NULL DEFAULT FALSE,
    opening_balance_date TIMESTAMPTZ,
    opening_balance_by UUID,
    closing_started_at TIMESTAMPTZ,
    closing_started_by UUID,
    closed_at TIMESTAMPTZ,
    closed_by UUID,
    locked_at TIMESTAMPTZ,
    locked_by UUID,
    lock_reason TEXT,
    allow_adjustments BOOLEAN NOT NULL DEFAULT FALSE,
    adjustment_deadline DATE,
    total_debits NUMERIC NOT NULL DEFAULT 0,
    total_credits NUMERIC NOT NULL DEFAULT 0,
    journal_count INTEGER NOT NULL DEFAULT 0,
    total_revenue NUMERIC NOT NULL DEFAULT 0,
    total_expenses NUMERIC NOT NULL DEFAULT 0,
    net_income NUMERIC NOT NULL DEFAULT 0,
    total_assets NUMERIC NOT NULL DEFAULT 0,
    total_liabilities NUMERIC NOT NULL DEFAULT 0,
    total_equity NUMERIC NOT NULL DEFAULT 0,
    balance_sheet_generated BOOLEAN NOT NULL DEFAULT FALSE,
    income_statement_generated BOOLEAN NOT NULL DEFAULT FALSE,
    statements_generated_at TIMESTAMPTZ,
    notes TEXT,
    metadata JSONB NOT NULL DEFAULT '{"created_at":null,"updated_at":null,"deleted_at":null,"created_by":null,"updated_by":null,"deleted_by":null}'::jsonb,
    PRIMARY KEY (id)
);

CREATE UNIQUE INDEX IF NOT EXISTS idx_fiscal_periods_provider_id_period_code ON fiscal_periods (provider_id, period_code) WHERE deleted_at IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_fiscal_periods_provider_id_fiscal_year_fiscal_month ON fiscal_periods (provider_id, fiscal_year, fiscal_month) WHERE deleted_at IS NULL AND fiscal_month IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_fiscal_periods_provider_id_fiscal_year ON fiscal_periods (provider_id, fiscal_year);

CREATE INDEX IF NOT EXISTS idx_fiscal_periods_provider_id_status ON fiscal_periods (provider_id, status);

CREATE INDEX IF NOT EXISTS idx_fiscal_periods_provider_id_start_date_end_date ON fiscal_periods (provider_id, start_date, end_date);

CREATE INDEX IF NOT EXISTS idx_fiscal_periods_provider_id_is_current ON fiscal_periods (provider_id, is_current);

CREATE INDEX IF NOT EXISTS idx_fiscal_periods_parent_id ON fiscal_periods (parent_id);

-- GIN index for audit metadata JSONB queries
CREATE INDEX IF NOT EXISTS idx_fiscal_periods_metadata_gin ON fiscal_periods USING GIN (metadata);
CREATE INDEX IF NOT EXISTS idx_fiscal_periods_metadata_deleted_at ON fiscal_periods ((metadata->>'deleted_at'));

-- Triggers for automatic metadata timestamp management
-- Automatically sets created_at on INSERT and updated_at on UPDATE

-- Function to set metadata->'created_at' on INSERT
CREATE OR REPLACE FUNCTION fiscal_periods_audit_timestamp() RETURNS trigger AS $$
BEGIN
    IF TG_OP = 'INSERT' THEN
        NEW.metadata = jsonb_set(NEW.metadata::jsonb, '{created_at}', to_jsonb(NOW()));
        NEW.metadata = jsonb_set(NEW.metadata::jsonb, '{updated_at}', to_jsonb(NOW()));
    ELSIF TG_OP = 'UPDATE' THEN
        NEW.metadata = jsonb_set(NEW.metadata::jsonb, '{updated_at}', to_jsonb(NOW()));
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Trigger to set timestamps on INSERT
DROP TRIGGER IF EXISTS fiscal_periods_insert_audit ON fiscal_periods;
CREATE TRIGGER fiscal_periods_insert_audit BEFORE INSERT ON fiscal_periods
    FOR EACH ROW EXECUTE FUNCTION fiscal_periods_audit_timestamp();

-- Trigger to set updated_at on UPDATE
DROP TRIGGER IF EXISTS fiscal_periods_update_audit ON fiscal_periods;
CREATE TRIGGER fiscal_periods_update_audit BEFORE UPDATE ON fiscal_periods
    FOR EACH ROW EXECUTE FUNCTION fiscal_periods_audit_timestamp();

