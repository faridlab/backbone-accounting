//! Domain policy for BankCheckSequence aggregate
//!
...
/// Domain policy for BankCheckSequence — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<BankCheckSequence>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type BankCheckSequenceDomainPolicy = PermitAllPolicy<BankCheckSequence>;
