//! Domain policy for Ledger aggregate
//!
...
/// Domain policy for Ledger — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Ledger>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type LedgerDomainPolicy = PermitAllPolicy<Ledger>;
