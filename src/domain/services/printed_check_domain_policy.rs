//! Domain policy for PrintedCheck aggregate
//!
...
/// Domain policy for PrintedCheck — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PrintedCheck>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PrintedCheckDomainPolicy = PermitAllPolicy<PrintedCheck>;
