//! Domain policy for Journal aggregate
//!
...
/// Domain policy for Journal — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Journal>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type JournalDomainPolicy = PermitAllPolicy<Journal>;
