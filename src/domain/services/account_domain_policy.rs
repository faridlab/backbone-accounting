//! Domain policy for Account aggregate
//!
...
/// Domain policy for Account — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Account>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type AccountDomainPolicy = PermitAllPolicy<Account>;
