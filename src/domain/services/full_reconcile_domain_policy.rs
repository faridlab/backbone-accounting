//! Domain policy for FullReconcile aggregate
//!
...
/// Domain policy for FullReconcile — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<FullReconcile>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type FullReconcileDomainPolicy = PermitAllPolicy<FullReconcile>;
