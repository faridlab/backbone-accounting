//! Application service for CostCenter entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `cost_center_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateCostCenterDto, UpdateCostCenterDto};
use crate::domain::entity::CostCenter;
use crate::infrastructure::persistence::CostCenterRepository;

/// Application service for CostCenter entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `cost_center_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct CostCenterServiceCustom {
///     inner: Arc<CostCenterService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type CostCenterService = GenericCrudService<
    CostCenter,
    CreateCostCenterDto,
    UpdateCostCenterDto,
    CostCenterRepository,
>;

// <<< CUSTOM
// END CUSTOM
