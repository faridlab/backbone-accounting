//! Repository for CostCenter entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Thin newtype over `backbone_orm::GenericCrudRepository<CostCenter, backbone_orm::SoftDelete>`.
//! All standard CRUD methods are available via `Deref`.

use anyhow::Result;
use sqlx::PgPool;

use crate::domain::entity::CostCenter;

/// Table name for CostCenter entities
pub const TABLE_NAME: &str = "cost_centers";

/// Repository for CostCenter entities.
///
/// All standard CRUD, soft-delete, pagination, and bulk methods are
/// provided automatically via `Deref` to `backbone_orm::GenericCrudRepository`.
pub struct CostCenterRepository(
    backbone_orm::GenericCrudRepository<CostCenter, backbone_orm::SoftDelete>,
);

impl std::ops::Deref for CostCenterRepository {
    type Target = backbone_orm::GenericCrudRepository<CostCenter, backbone_orm::SoftDelete>;
    fn deref(&self) -> &Self::Target { &self.0 }
}

impl CostCenterRepository {
    /// Create a new repository instance.
    pub fn new(pool: PgPool) -> Self {
        Self(backbone_orm::GenericCrudRepository::new(pool, TABLE_NAME))
    }
}

backbone_core::impl_crud_repository!(CostCenterRepository, CostCenter, soft_delete);
