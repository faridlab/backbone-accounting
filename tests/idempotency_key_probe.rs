...
async fn ikp1_same_key_dedups() {
    ...
    let a = svc
        .post(
            balanced(company, Uuid::new_v4(), exp, bank, dec("1000")).with_idempotency_key(&key),
            None,
        )
        .await
        .unwrap();
...
async fn ikp2_distinct_keys_same_source_id_both_post() {
    ...
    let a = svc
        .post(
            balanced(company, one_document, exp, bank, dec("1000"))
                .with_idempotency_key(format!("{one_document}:consume")),
            None,
        )
        .await
        .unwrap();
    let b = svc
        .post(
            balanced(company, one_document, exp, bank, dec("2000"))
                .with_idempotency_key(format!("{one_document}:receive")),
            None,
        )
        .await
        .unwrap();
    ...
    assert!(
        !b.idempotent_reuse,
        "a distinct key posts a distinct journal even for the same source_id"
    );
    assert_ne!(
        a.journal_id, b.journal_id,
        "TWO journals — the multi-post producer no longer loses the 2nd"
    );
}
...
async fn ikp3_keyless_still_dedups_on_tuple() {
    ...
    let a = svc
        .post(balanced(company, source, exp, bank, dec("1000")), None)
        .await
        .unwrap();
    let b = svc
        .post(balanced(company, source, exp, bank, dec("1000")), None)
    ...
    assert!(
        b.idempotent_reuse,
        "keyless: same tuple still dedups (backward compatible)"
    );
